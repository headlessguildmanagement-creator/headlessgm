  ...
  return (
    <AppShell guildName={guild.name} guildSlug={guild.slug} eyebrow="AUCTION" title="Auction Operations" activeHref="/app/auctions">
      <section className="panel">
        <div className="panel-pad section-head"><div><h2>Event auctions</h2><p>Generate draft → officer review → publish → finalize. Persistent rotations advance only on finalization.</p></div><Link href={withGuild('/app/settings/auction', guild.slug)} className="button ghost">Rules & caps</Link></div>
          ...
          <table>
            ...
            <tbody>
              {(events || []).map((event) => {
                ...
                return <tr key={event.id}><td><strong>{event.name}</strong><div className="muted" style={{ fontSize: 12 }}>{event.event_type.replaceAll('_',' ')}</div></td><td>{new Date(event.starts_at).toLocaleString()}</td><td><span className="pill">{event.status}</span></td><td><span className="pill">{run?.status || 'not generated'}</span></td><td>{run ? total : '—'}</td><td><Link href={withGuild(`/app/events/${event.id}/auction`, guild.slug)}>Open →</Link></td></tr>
              })}
