...
export default async function LinkCharacterPage({ searchParams }) {
  ...
  const discordUserId = discordIdentity?.identity_data?.provider_id || discordIdentity?.provider_id || ''
  if (!discordUserId) {
    return (
      ...
      <div style={{ width: 'min(760px, 100%)', margin: '0 auto', display: 'grid', gap: 24 }}>
        <header>
          ...
          <p className="lede" style={{ fontSize: 18, marginTop: 14 }}>
            Choose the existing HeadlessGM roster record that belongs to your Discord identity. Username or nickname is never used as the link key.
          </p>
        </header>
        ...
        {membersError ? (
          <div style={{ border: '1px solid #7f1d1d', borderRadius: 12, padding: 14, color: '#fca5a5' }}>Could not load the available roster.</div>
        ) : members?.length ? (
          <section style={{ display: 'grid', gap: 12 }}>
            {members.map((member) => (
              <form key={member.id} action={submitCharacterClaim} style={{ border: '1px solid #232832', borderRadius: 14, background: '#11151a', padding: 18, display: 'grid', gridTemplateColumns: '1fr auto', gap: 18, alignItems: 'center' }}>
                ...
                <div>
                  <strong style={{ fontSize: 18 }}>{member.ign}</strong>
                  <div style={{ color: '#8893a1', fontSize: 13, marginTop: 5 }}>{jobLabels.get(member.job_code) || member.job_code || 'Job not set'} · {member.status}</div>
                </div>
          ...
          <section style={{ border: '1px solid #232832', borderRadius: 14, background: '#11151a', padding: 24 }}>
            <strong>No unlinked characters are available.</strong>
            <p style={{ color: '#8893a1', marginBottom: 0 }}>An officer may need to add your character to the HeadlessGM roster first, or another claim may already be pending.</p>
          </section>
        )}
      </div>
    </main>
