...
function parseXml(text) {
  ...
  return members.map((match) => ({
    ign: xmlValue(match[1], ['ign','name']),
    class: xmlValue(match[1], ['class','job','jobclass']),
    combatrole: xmlValue(match[1], ['combatrole','combat_role','role']),
    guildrank: xmlValue(match[1], ['guildrank','guild_rank','rank']),
    feathergroup: xmlValue(match[1], ['feathergroup','feather_group','group']),
    puppetorder: xmlValue(match[1], ['puppetorder','puppet_order']),
  }))
}
...
export async function importMembers(formData) {
  ...
  if (file.size > 1024 * 1024) redirect('/app/members?error=Roster%20file%20must%20be%201%20MB%20or%20smaller.')
  ...
  const [{ data: jobs }, { data: existing }, { data: rules }, limit, currentCount] = await Promise.all([
    supabase.from('game_jobs').select('code,label').eq('game_preset_id', guild.game_preset_id).eq('is_active', true),
    supabase.from('guild_members').select('ign').eq('guild_id', guild.id).in('status', ['active','pending']),
  ...
  const existingIgns = new Set((existing || []).map((row) => norm(row.ign)))
  ...
  rawRows.forEach((row, index) => {
    ...
    const combatRole = pick(row, ['CombatRole','combat_role']) || null
    ...
    const order = puppetOrder(pick(row, ['PuppetOrder','puppet_order']))
    const jobCode = jobMap.get(norm(jobRaw)) || null

    if (!ign) errors.push(`Row ${line}: IGN is required.`)
    else if (existingIgns.has(norm(ign)) || seenIgns.has(norm(ign))) errors.push(`Row ${line}: duplicate IGN ${ign}.`)
    ...
    rows.push({ guild_id: guild.id, ign, job_code: jobCode, combat_role: combatRole, guild_role: guildRole, feather_group: Number.isNaN(group) ? null : group, puppet_order: Number.isNaN(order) ? null : order, is_officer: /officer|commander|vice|guild leader|leader/i.test(guildRole), status: 'active' })
  })

  if (errors.length) redirect(`/app/members?error=${safe(errors.slice(0, 5).join(' '))}`)
  if (currentCount + rows.length > limit) redirect(`/app/members?error=${safe(`Import would exceed the active member limit of ${limit}. Current active roster: ${currentCount}.`)}`)

  const { error } = await supabase.from('guild_members').insert(rows)
  if (error) redirect(`/app/members?error=${safe(error.message || 'Roster import failed.')}`)

  revalidatePath('/app/members')
  revalidatePath('/app')
  redirect(`/app/members?success=${safe(`${rows.length} members imported successfully.`)}`)
}
...
export async function addMember(formData) {
  ...
  const { error } = await supabase.from('guild_members').insert({ guild_id: guild.id, ign, job_code: jobCode, combat_role: combatRole, guild_role: guildRole, is_officer: /officer|commander|vice|guild leader|leader/i.test(guildRole || ''), status })
  if (error) redirect('/app/members?error=Could%20not%20add%20that%20member.')
  revalidatePath('/app/members')
...
export async function updateMemberStatus(formData) {
  ...
  if (requestedStatus === 'active') {
    const { data: member } = await supabase.from('guild_members').select('status').eq('id', memberId).eq('guild_id', guild.id).single()
