...
export default async function AuctionSettingsPage({ searchParams }) {
  ...
  const paid = guild.plan_code !== 'free'
      ...
      <form action={saveAuctionRules} style={{ display: 'grid', gap: 18 }}>
        ...
        <section className="panel panel-pad">
          ...
          <div className="form-grid">
            <label className="field"><span>Feather allocation</span><select name="feather_mode" defaultValue={rules?.feather_mode || 'ffa'}><option value="ffa">Free For All</option>{paid ? <option value="four_group">4 Group Rotation · GUILD</option> : null}<option value="random">Random among eligible members</option><option value="custom" disabled={!commander}>Custom — COMMANDER</option></select></label>
            <label className="field"><span>Puppet allocation</span><select name="puppet_mode" defaultValue={rules?.puppet_mode || 'round_robin'}><option value="ffa">Free For All</option>{paid ? <option value="round_robin">Round Robin · GUILD</option> : null}<option value="random">Random among eligible members</option><option value="custom" disabled={!commander}>Custom — COMMANDER</option></select></label>
          </div>
          {guild.plan_code === 'free' ? <p className="muted" style={{ marginBottom: 0 }}>FREE keeps auctions simple with FFA and Random. Upgrade to GUILD for persistent Feather/Puppet presets and reward caps.</p> : !commander ? <p className="muted" style={{ marginBottom: 0 }}>GUILD includes proven standard presets. Custom allocation logic is available on COMMANDER.</p> : null}
        </section>

        {paid ? <section className="panel panel-pad">
        ...
        </section> : <section className="panel panel-pad"><p className="eyebrow">GUILD</p><h2>Reward caps and persistent auction presets</h2><p className="muted">Upgrade to GUILD to use per-person caps, 4 Group Feather and Puppet Round Robin.</p></section>}

        {paid ? <section className="panel panel-pad">
          <div className="section-head"><div><h2>ROOC persistent rules</h2><p>When the standard ROOC preset is used, HeadlessGM keeps the persistent operating state instead of rebuilding it per event.</p></div></div>
