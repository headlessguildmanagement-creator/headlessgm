...
export async function startCheckout(formData) {
  ...
  if (existing?.provider_subscription_id && !['expired'].includes(String(existing.status))) {
    redirect(`/${guild.slug}/settings/billing?error=${safe('This guild already has a subscription. Use Manage billing to change or cancel it.')}`)
  }
  ...
  const response = await fetch('https://api.lemonsqueezy.com/v1/checkouts', {
    ...
    body: JSON.stringify({
      data: {
        ...
        attributes: {
          product_options: {
            redirect_url: `${baseUrl()}/${guild.slug}/settings/billing?checkout=success`,
