...
export function entitlementPlanForEvent({ eventName, status, candidatePlan, currentPlan }) {
  ...
  // Lemon Squeezy changes the variant immediately even when a customer has
  // already paid for the higher tier through the renewal date. Preserve the
  // current entitlement until the next successful payment confirms the target
  // tier. Expiration remains the only paid-to-FREE transition.
  return current
}
