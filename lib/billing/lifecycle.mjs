...
export function webhookEventKey(eventName, payload) {
  const type = String(payload?.data?.type || 'unknown')
  const id = String(payload?.data?.id || 'unknown')
  return `lemonsqueezy:${String(eventName || 'unknown')}:${type}:${id}`
}
