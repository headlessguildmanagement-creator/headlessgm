...
export async function provisionHeadlessGMChannels(discordGuildId) {
  ...
  const auctionProof = await ensureText('auction-proof', 'Post IGN: YourName with a screenshot, then use Apps → Submit Auction Proof, or use /auction-proof. HeadlessGM stores proof status only, not the screenshot.')
...
export async function registerAuctionProofCommand(discordGuildId) {
  ...
  for (const command of commands) {
    const current = existing.find((item) => item.name === command.name && Number(item.type || 1) === Number(command.type || 1))
    if (current?.id) {
      results.push(await discordRequest(`${path}/${current.id}`, { method: 'PATCH', body: JSON.stringify(command) }))
    } else {
      results.push(await discordRequest(path, { method: 'POST', body: JSON.stringify(command) }))
    }
  }
  return results
}
